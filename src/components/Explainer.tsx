...
export function Explainer({ onOpenExplanation }: { onOpenExplanation?: () => void }) {
  ...
  const searchCopy: Record<string, { title: string; detail: string }> = {
    k2: { title: current.graph.entry === null ? 'No dots to search yet.' : 'Start at the top of the map.', detail: current.graph.entry === null ? 'Add some dots in Explore first.' : guided ? 'This example has only layer 0. Start at S (Slow drums). A is Acoustic guitar, B is Brass band, T is Trumpet solo; the pink target is the new song. Predict which linked song will be checked next, then press Next.' : 'Every search starts at the same dot on the highest layer. From here, it looks for routes toward your target.' },
  ...
  return (
    <div className="explainer">
      ...
      <p><RichText text={updateDone && before && after
      ...
      {insertionLevel && <details className="caption-detail"><summary>How the random level is calculated</summary><p><RichText text={current.detail}/></p></details>}
      {updateDone && <button className="learn-link" onClick={() => dispatch({ type: 'seek', index: Math.max(0, trace.steps.findIndex(s => s.line === 'i3')) })}>Inspect the new level decision →</button>}
      {guided && step === trace.steps.length - 1 && <button className="learn-link" onClick={() => dispatch({ type: 'setRightTab', tab: params.efSearch === 1 ? 'params' : 'metrics' })}>{params.efSearch === 1 ? 'Next: set efSearch to 2, then rerun this target →' : 'Compare the results: why did keeping B help? →'}</button>}
    </div>
  )
}
