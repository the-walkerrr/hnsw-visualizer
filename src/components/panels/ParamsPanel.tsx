...
export function ParamsPanel() {
  ...
  const reset = () => { setFeedback({}); dispatch({ type: 'setParams', patch: { ...DEFAULT_PARAMS } }) }
  ...
  <fieldset className="pane-scroll panel-fields" disabled={editsLocked(state)}>
    ...
    <div className="row dataset-actions">
      <button className="button secondary compact" onClick={() => script([{ t: 'tool', tool: 'insert' }, { t: 'insert', at: [...randomPoint()] }, { t: 'seek', to: 'end' }])}>Add one dot</button>
      <AlertDialog.Root>
        <AlertDialog.Trigger asChild><button className="button ghost compact" disabled={!vectorCount}>Clear dots</button></AlertDialog.Trigger>
        <AlertDialog.Portal>
          <AlertDialog.Overlay className="dialog-overlay" />
          <AlertDialog.Content className="dialog-content">
            <AlertDialog.Title className="dialog-title">Clear every dot?</AlertDialog.Title>
            <AlertDialog.Description className="dialog-description">This removes all dots, edges, and the current replay. This cannot be undone.</AlertDialog.Description>
            <div className="dialog-actions">
              <AlertDialog.Cancel asChild><button className="button compact">Cancel</button></AlertDialog.Cancel>
              <AlertDialog.Action asChild><button className="button compact danger" onClick={() => script([{ t: 'clear' }])}>Clear dots</button></AlertDialog.Action>
            </div>
          </AlertDialog.Content>
        </AlertDialog.Portal>
      </AlertDialog.Root>
    ...
    <Slider {...feedbackProps('param-M')} id="param-M" label="Connections (M)" value={params.M} min={2} max={24} hint={`Each new dot keeps up to ${params.M} chosen neighbors per layer. This demo also resets both degree caps and mL when M changes: caps ${params.M} and ${params.M * 2}; mL ${(1 / Math.log(Math.max(params.M, 2))).toFixed(2)}. Changing M rebuilds the graph.`} guide="M" onChange={(M) => { setFeedback((current) => ({ ...current, Mmax: undefined, Mmax0: undefined, mL: undefined })); set({ M, Mmax: M, Mmax0: M * 2, mL: 1 / Math.log(Math.max(M, 2)) }) }}/>
    <Slider {...feedbackProps('efc')} id="efc" label="Build effort (efConstruction)" value={params.efConstruction} min={1} max={200} hint="Possible neighbors considered while adding a dot." guide="efConstruction" onChange={(efConstruction) => set({ efConstruction })}/>
    <details className="advanced-details"><summary>Advanced settings <span>Optional</span></summary><div className="details-body">
    <Slider {...feedbackProps('mL')} id="mL" label="Layer multiplier (mL)" value={params.mL} min={0.1} max={2} step={0.01} format={(v) => v.toFixed(2)} hint={`Usual value for M ${params.M}: ${(1 / Math.log(Math.max(params.M, 2))).toFixed(2)}.`} guide="mL" onChange={(mL) => set({ mL })}/>
    <div className="field"><div className="field-head"><label htmlFor="metric">Distance metric</label></div><select id="metric" value={params.metric} onChange={(e) => set({ metric: e.target.value as Metric })}>{(Object.keys(METRIC_LABEL) as Metric[]).map((metric) => <option key={metric} value={metric}>{METRIC_LABEL[metric]}</option>)}</select><p className="hint">{METRIC_NOTE[params.metric]}</p><ControlHelp guide="metric"/></div>
      ...
      <Slider {...feedbackProps('Mmax')} id="Mmax" label="Maximum degree above L0" value={params.Mmax} min={params.M} max={32} hint={`Hard edge limit; cannot be lower than M (${params.M}).`} guide="Mmax" onChange={(Mmax) => set({ Mmax })}/>
      <Slider {...feedbackProps('Mmax0')} id="Mmax0" label="Maximum degree at L0" value={params.Mmax0} min={params.M} max={64} hint="Hard edge limit on the bottom layer; usually 2 × M." guide="Mmax0" onChange={(Mmax0) => set({ Mmax0 })}/>
