...
export const CONTROL_GUIDES = {
  dataset: {
    ...
    example: 'Keep the count at 48 and switch from Uniform to Clusters. The dots regroup and the index rebuilds; a search may now spend longer inside one dense group before finding a bridge out.',
  ...
  vectors: {
    ...
    example: 'At 24 vectors, most links and search decisions remain easy to follow. At 80, the same dataset shape has more stored dots, more edges, and longer traces.',
  ...
  k: {
    ...
    inputs: [
      ...
      { label: 'How to choose', explanation: 'Use 1 to inspect the single nearest result, or a larger value when you want a broader result set. Larger requests may need a larger SEARCH_WIDTH.' },
    ],
    example: 'With the same target, NUM_RESULTS_REQUESTED = 1 returns only the closest match; NUM_RESULTS_REQUESTED = 5 returns up to five matches. The stored graph does not change.',
    lower: 'A smaller result count asks for fewer answers and is easier to inspect.',
    higher: 'A larger result count asks for more answers and may require a wider search to find all of them.',
    when: 'This affects the next search only; it does not rebuild the graph.',
  },
  M: {
    id: 'control-m',
    label: 'Target connections per node',
    term: 'TARGET_CONNECTIONS',
    plain: 'How many neighbors a new dot tries to keep on every layer where it appears.',
    inputs: [
      { label: 'Input: 2–24', explanation: 'Enter a whole number for the target neighbors chosen per layer. TARGET_CONNECTIONS = 5 means up to five chosen neighbors on every layer containing the new dot—not exactly five when fewer useful candidates exist.' },
    ...
    example: 'With TARGET_CONNECTIONS = 2, a new dot tries to keep two neighbors per layer. With TARGET_CONNECTIONS = 8, it can keep eight, creating more routes but using more memory and build work.',
    ...
    when: 'Changing TARGET_CONNECTIONS rebuilds the graph. This playground also resets the connection limits and layer multiplier to matching defaults.',
  },
  efConstruction: {
    id: 'control-ef-construction',
    label: 'Build search width',
    term: 'BUILD_WIDTH',
    ...
    example: 'BUILD_WIDTH = 20 keeps a small candidate pool while inserting each dot. Raising it to 100 considers more possible links before choosing, so building costs more but later searches often find better routes.',
    ...
    learnMore: { href: '#chapter-insert', label: 'See how build width is used during insertion' },
  },
  mL: {
    id: 'control-ml',
    label: 'Layer multiplier',
    term: 'LAYER_MULTIPLIER',
    plain: 'Controls how likely a dot is to appear on the sparse upper “express-lane” layers.',
    inputs: [
      { label: 'Input: 0.10–2.00', explanation: 'Enter a decimal in steps of 0.01. The usual starting value is 1 / ln(TARGET_CONNECTIONS).' },
    ...
    example: 'For TARGET_CONNECTIONS = 5, the usual starting value is 1 / ln(5), about 0.62. Raising LAYER_MULTIPLIER to 1.20 makes dots more likely to reach upper layers, producing a taller, denser hierarchy.',
    ...
    when: 'Build-time setting: changing it rebuilds the graph. The usual value is 1 / ln(TARGET_CONNECTIONS).',
  },
  metric: {
    id: 'control-metric',
    ...
    example: 'From QUERY = (0, 0), item A = (3, 0), and item B = (2, 2): Euclidean says B is closer (2.83 vs 3), while Manhattan says A is closer (3 vs 4). Changing the metric can therefore change both links and results.',
  ...
  efSearch: {
    ...
    inputs: [
      { label: 'Input: 1–200', explanation: 'The effective capacity is max(SEARCH_WIDTH, NUM_RESULTS_REQUESTED). If NUM_RESULTS_REQUESTED is 5, SEARCH_WIDTH values 1 through 5 all use five slots. This is not a limit on distance checks or the number of final results.' },
    ...
    example: 'With NUM_RESULTS_REQUESTED = 1 and SEARCH_WIDTH = 1, the search may discard a slightly worse-looking route too early. At SEARCH_WIDTH = 2, it can keep that route long enough to discover the true nearest dot; the result count is still one.',
    ...
    learnMore: { href: '#chapter-search', label: 'See how search width controls a query' },
  },
  Mmax: {
    id: 'control-mmax',
    label: 'Maximum connections above the base layer',
    term: 'MAX_CONNECTIONS_UPPER',
    plain: 'The hard limit on how many edges one dot may keep on an upper layer.',
    inputs: [
      { label: 'Input: target–32', explanation: 'Enter a whole number from the current TARGET_CONNECTIONS value through 32; this limit cannot be lower than the target.' },
      { label: 'What the value means', explanation: 'Values close to TARGET_CONNECTIONS keep upper layers sparse. Larger values preserve more alternate express-lane routes, using more memory and traversal work.' },
    ],
    example: 'With TARGET_CONNECTIONS = 5 and MAX_CONNECTIONS_UPPER = 8, each new upper-layer dot targets five chosen neighbors, while an existing upper-layer dot may retain at most eight connections after pruning.',
    ...
    when: 'Build-time setting: changing it rebuilds the graph. It cannot be lower than TARGET_CONNECTIONS.',
  },
  Mmax0: {
    id: 'control-mmax0',
    label: 'Maximum connections on the base layer',
    term: 'MAX_CONNECTIONS_BASE',
    plain: 'The hard limit on edges per dot on the bottom layer, where every dot lives.',
    inputs: [
      { label: 'Input: target–64', explanation: 'Enter a whole number from the current TARGET_CONNECTIONS value through 64. A common starting point is 2 × TARGET_CONNECTIONS.' },
      { label: 'What the value means', explanation: 'Values close to TARGET_CONNECTIONS keep the largest layer compact. Larger values retain more local routes and can improve recall, but add edges at the most expensive layer.' },
    ],
    example: 'With TARGET_CONNECTIONS = 5 and MAX_CONNECTIONS_BASE = 10, new dots target five bottom-layer neighbors, but an existing bottom-layer dot can keep up to ten connections as other dots link back to it.',
    ...
    when: 'Build-time setting: changing it rebuilds the graph. A common default is 2 × TARGET_CONNECTIONS.',
  },
  seed: {
    id: 'control-seed',
    ...
    example: 'Seed 7 always gives the same dots the same random layer assignments. Change it to 8 and the dot positions stay fixed, but the hierarchy and links are rebuilt differently.',
  ...
  neighborRule: {
    ...
    example: 'Suppose TARGET_CONNECTIONS = 2 and the two closest candidates are both east of N, while a slightly farther candidate is west. Simple keeps both eastern links; Heuristic may keep one east and one west to preserve two directions.',
  ...
  extendCandidates: {
    ...
    example: 'If candidate A links to B but B was not in N’s original candidate pool, Off never considers B. On adds B to the pool through A, giving N one more possible connection.',
  ...
  keepPrunedConnections: {
    ...
    inputs: [
      { label: 'Example: TARGET_CONNECTIONS = 3', explanation: 'Suppose node N considers items A, B, and C. A and B point in nearly the same direction, while C points elsewhere. The heuristic keeps A and C, then prunes B because A already covers that direction. Off leaves N with two links. On uses the empty third slot for B.' },
      { label: 'Off', explanation: 'Keeps only diverse edges, even when that leaves unused edge slots and some dots under-connected.' },
      { label: 'On', explanation: 'Uses the closest rejected candidates to fill open slots. It never exceeds TARGET_CONNECTIONS; it only uses link capacity that would otherwise remain empty.' },
    ],
    example: 'With TARGET_CONNECTIONS = 3, the heuristic keeps A and C but rejects nearby B as redundant. Off leaves the third slot empty; On fills that unused slot with B, so N has three links instead of two.',
