...
export const CONTROL_GUIDES = {
  ...
  k: {
    ...
    example: 'With the same target, k = 1 returns only the closest match; k = 5 returns up to five matches. The stored graph does not change.',
  ...
  M: {
    ...
    example: 'With M = 2, a new dot tries to keep two neighbors per layer. With M = 8, it can keep eight, creating more routes but using more memory and build work.',
  ...
  efConstruction: {
    ...
    example: 'efConstruction = 20 keeps a small candidate pool while inserting each dot. Raising it to 100 considers more possible links before choosing, so building costs more but later searches often find better routes.',
  ...
  mL: {
    ...
    example: 'For M = 5, the usual starting value is 1 / ln(5), about 0.62. Raising mL to 1.20 makes dots more likely to reach upper layers, producing a taller, denser hierarchy.',
  ...
  metric: {
    ...
    example: 'From q = (0, 0), A = (3, 0) and B = (2, 2): Euclidean says B is closer (2.83 vs 3), while Manhattan says A is closer (3 vs 4). Changing the metric can therefore change both links and results.',
  ...
  efSearch: {
    ...
    example: 'With k = 1 and efSearch = 1, the search may discard a slightly worse-looking route too early. At efSearch = 2, it can keep that route long enough to discover the true nearest dot; the result count is still one.',
  ...
  Mmax: {
    ...
    example: 'With M = 5 and Mmax = 8, each new upper-layer dot targets five chosen neighbors, while an existing upper-layer dot may retain at most eight incoming and outgoing connections after pruning.',
  ...
  Mmax0: {
    ...
    example: 'With M = 5 and Mmax0 = 10, new dots target five bottom-layer neighbors, but an existing bottom-layer dot can keep up to ten connections as other dots link back to it.',
  ...
  neighborRule: {
    ...
    example: 'Suppose M = 2 and the two closest candidates are both east of N, while a slightly farther candidate is west. Simple keeps both eastern links; Heuristic may keep one east and one west to preserve two directions.',
  ...
  keepPrunedConnections: {
    ...
    example: 'With M = 3, the heuristic keeps A and C but rejects nearby B as redundant. Off leaves the third slot empty; On fills that unused slot with B, so N has three links instead of two.',
