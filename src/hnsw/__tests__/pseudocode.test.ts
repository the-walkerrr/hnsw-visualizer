...
describe('learner-facing pseudocode', () => {
  it('removes each discarded candidate before refilling the result set', () => {
    expect(line('select-neighbors', 'h11').text).toContain('remove nearest from REJECTED_CANDIDATES')
  })
  ...
  it('uses the bottom-layer degree cap in insert and hard-delete repair', () => {
    ...
    expect(insert).toContain('CONNECTION_LIMIT(CURRENT_LAYER) ← MAX_CONNECTIONS_BASE on layer 0; otherwise MAX_CONNECTIONS_UPPER')
    expect(insert).toContain('> CONNECTION_LIMIT(CURRENT_LAYER)')
    expect(deletion).toContain('CONNECTION_LIMIT(CURRENT_LAYER)')
  })
